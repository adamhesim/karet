 ...
 */
class MustacheLogger extends Mustache_Logger_AbstractLogger
{
    ...
    protected static $log4Func = array(
        self::DEBUG     => 'debug',
        self::INFO      => 'info',
        self::NOTICE    => 'info',
        self::WARNING   => 'warn',
    ...
    {
        $this->setLevel($level);
        $this->log4php= Logger::getLogger('myLogger');
    }
     ...
     *
     * @throws Mustache_Exception_RuntimeException If the logger cannot be opened.
     *
    ...
    {
        if (!is_a($this->log4php,'Logger')) {
            throw new Mustache_Exception_RuntimeException('Unable to open Logger');
        }

        $func = self::$log4Func[$level];
        $this->log4php->$func(self::formatLine($level, $message, $context));
    }
